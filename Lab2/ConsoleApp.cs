...
static class ProcessArguments
{
    private static Customer ParseArguments(CustomerRepository repo)
    {
        ...
        ValidateCustomer(userName, password, repo);
        Customer currentCustomer = ValidateCustomer(userName, password, repo);

        return currentCustomer;
    }

    private static Customer ValidateCustomer(string userName, string password, CustomerRepository repo)
    {
        Customer currentCustomer = repo.GetByUserName(userName);
        if (currentCustomer == null)
        {
            throw new ArgumentException($"Incorrect password '{password}' or '{userName}'");
        }
        return currentCustomer;
    ...
    public static void Run(ShopService shopService)
    {
        ...
        while (true)
        {
            ...
            try
            {
                ...
                Console.WriteLine($"Command '{command}' was finished in {commandWatch.ElapsedMilliseconds} ms.");
            }
            catch (Exception ex)
            {
