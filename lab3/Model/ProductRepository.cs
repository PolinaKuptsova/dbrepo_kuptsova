...
public class ProductRepository
{
    ...
    public List<Product> GetSortedProductsInOrder(long order_id, double low, double high)
    {
        ...
        NpgsqlCommand command = this.connection.CreateCommand();
        command.CommandText =
        @"CREATE INDEX ix_prodName ON name USING gin (to_tsvector('english', name));
    ...
    public void ActivateTrigger1(Product product)
    {
        NpgsqlCommand command = this.connection.CreateCommand();
        command.CommandText = @" CREATE TRIGGER  tg_products AFTER UPDATE ON products
                            BEGIN
                            UPDATE history SET createdAt = &createdAt WHERE product_id = $product_id
                            END";
    ...
    {
        NpgsqlCommand command = this.connection.CreateCommand();
        command.CommandText = @"CREATE TRIGGER Products_INSERT
                                ON products
                                AFTER INSERT
                                AS
