...
public class CustomerRepository
{
    ...
    public long GetCount()
    {
        ...
        command.CommandText = @"CREATE INDEX ix_id ON customers(id) ;
    ...
    public int Insert(Customer customer)
    {
        NpgsqlCommand command = this.connection.CreateCommand();
        command.CommandText =
        @"INSERT INTO customers (phoneNumber, address, userName, status) 
            VALUES ($phoneNumber, $address, $userName, $status);
    ...
    public List<Customer> GetSelectedCustomer(long id)
    {
        List<Customer> customers = new List<Customer>();
        NpgsqlCommand command = this.connection.CreateCommand();
        command.CommandText = @"CREATE INDEX ix_customersId ON customers USING BTREE(id);
            SELECT * FROM customers WHERE id = $id AND customers.id  > 20 
